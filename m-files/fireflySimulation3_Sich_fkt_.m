function [phaseOut,omegaOut,phi_ext] = fireflySimulation3_Sich_fkt_(varargin)
...
%                 fire(1)=mod(f(1)+1,2);
%                 in=in+1;
%                 interactions(in,[1:3,11]) = [2 i j fire(j)];
%             else
%                 fire(1)=0;
%                 test(i) = f(1);
            end
...
%        fprintf('time %d - phi(%d,:)=[%f %f], node %d\n',i,i, phi(i,:),j) % (for n=2 only)
         if phi_ext(i) >= 1        % if external node is at maximum
           ...
           % f(j)=1;             % fire on every peak
            if f(j) == 0
                in=in+1;
                interactions(in,1:11) = [-1 i j 0 0 0 0 0 0 0 0];
            end
            if f(j) == 1 && fire == 1      % if node j fires and firing has already oCured at this time.
                in=in+1;
                interactions(in,1:3) = [0 i j];
            end

            if f(j) == 1 && fire == 0       % if node j fires and firing has not already oCured at this time.
...
%                         Ei{k}(length(Ei{k})+1) = i;
                     ...
                     if length(E) < FilterLength
%                         C(i:SimulationLength,k) = filterType(E{k}(1:length(E{k})));
%                         C(end+1) = filterType(E{k}(1:length(E{k})));
                        C(end+1) = filterType(E(1:length(E)));
                     else
%                         C(i:SimulationLength,k) = filterType(E{k}(length(E{k})-FilterLength+1:length(E{k})));
%                         C(end+1) = filterType(E{k}(length(E{k})-FilterLength+1:length(E{k})));
                        C(end+1) = filterType(E(length(E)-FilterLength+1:length(E)));
                     end
...
%                         %USE FILTERED SIN(E) INSTEAD OF CONFIDENCE domega(k,1) = ((omega(k) * 2^(adapting(k)*Beta*sin(2*pi*phi(i,k))*(filterType(E(length(E(:,k))-FilterLength+1:length(E(:,k)),k)))))+prod(domega(k,1:2)))/(domega(k,2)+1); % using sin(E) instead of
                        ...
                        domega(k,1) = ((omega(k) * 2^(Beta*-sin(2*pi*phi(i,k))*C(end)))+prod(domega(k,1:2)))/(domega(k,2)+1);
%                        domega(k,1) = ((omega(k) * 2^(Beta*-sin(2*pi*phi(i,k))*(filterType(E(length(E(:,k))-FilterLength+1:length(E(:,k)),k)))))+prod(domega(k,1:2)))/(domega(k,2)+1);
                        ...
                        in=in+1;
                        interactions(in,1:9) = [1 i j k 1 C(end) phi(i,k)-phi(i-1,k) Beta*sin(2*pi*phi(i,k)) domega(k,1)];
                    else
                        in=in+1;
                        interactions(in,1:6) = [1 i j k 0 C(end)]; %col 5 == 0 means fire within tref
                    end
%                 end
            end

            if domega(j,2) > 0 % if there has bEn freq adjustments
...
%                 end
                in=in+1;
                interactions(in,1:10) = [-2 i j j 1 C(end) -1 0 0 omega(j)];
            end

            if length(E) < FilterLength
                allzeroes = sum(E(1:length(E)));
                %allzeroes = filterType(E{j}(1:length(E{j})));
            else
                allzeroes = sum(E(length(E)-FilterLength+1:length(E)));
            ...
            if allzeroes == 0 && length(interactions(interactions(:,3)==j & interactions(:,11)==1,1)) > 8
                ...
                in=in+1;
                interactions(in,1:10) = [-3 i j j 1 C(end) -1 0 0 omega(j)];
            end
        end
